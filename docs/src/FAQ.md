...
As a consequence, unless [font embedding](#customizing-fonts) is used, the layout logic uses a hard-coded
font advance width (*aka* char width), which may lead to various artifacts (e.g., interrupted underlines,
stretched / compressed text etc.). This is required to align background color boxes with the text.
...
[you can](examples/fonts.md#embedding-custom-fonts) embed fonts with the `--embed-font` flag.
...
[`Template`]: crates/term_transcript/svg/struct.Template.html
[`TemplateOptions`]: crates/term_transcript/svg/struct.TemplateOptions.html
[`FontEmbedder`]: crates/term_transcript/svg/trait.FontEmbedder.html
[`FontSubsetter`]: crates/term_transcript/svg/struct.FontSubsetter.html
